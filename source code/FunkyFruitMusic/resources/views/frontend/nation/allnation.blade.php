    ...
    <div class="container-fluid">
        <div class="row sider">
            <div class="col-2 sidebar">
                <a href="#" class="active-sidebar">All Nations</a>
            ...
            <div class="col-10 sidebar-content">
                @foreach ($nations as $nation)
                    <?php
                    $songs = DB::select('select s.id as id, s.name as name, s.file as file from nations c, songs s where c.id = s.nation_id and c.id = ? limit 5', [$nation->id]);
                    ?>
                    <h1><a href="{{  url('nation/'. $nation->id)}}" class="nonactive-sidebar">{{ $nation->name }} ></a></h1>
                    @if (count($songs))
